...
global_popular <- function(train , test , eval_method = mean_rank,...){
  ...
  pred_matrix <- matrix(nrow = length(unique(test$user)), ncol = length(unique(test$item)))
...
random_recom <- function(train = NA , test , eval_method = mean_rank,...){
  ...
  random_ratings <- runif(n = length(unique(test$item))*length(unique(test$user)), 
                          min = 0, max = 4)
  
  pred_matrix <- matrix(random_ratings, nrow = length(unique(test$user)), ncol = length(unique(test$item)))
  pred <- setNames(melt(pred_matrix), c('user','item','rating'))
...
kfoldcv <- function(fulldata, k = 5, seed = 123, model , eval_method = eval_ranking, type = "random",...){
  set.seed(seed)
